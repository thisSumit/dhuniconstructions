    ...
    useLayoutEffect(() => {
        ...
        gsap.to(logo.current, {
            scrollTrigger: {
                ...
                onLeave: () => {
                    gsap.to(logo.current, {background: "#04293A", duration: 0.25, boxShadow: "0 0 10px rgba(0, 0, 0, 0.1)", ease: "power2.out" });
                },
                onEnterBack: () => {
                    gsap.to(logo.current, {background: "transparent", duration: 0.25, boxShadow: "0 0 10px rgba(0, 0, 0, 0)", ease: "power2.out" });
                },
            },
        });
    ...
    return (
        <div className='w-screen top-0 left-0 flex flex-col fixed justify-between items-center z-30 bg-gray-800' ref={container}>
            <div ref={logo} className='py-4 px-4 md:px-6 text-black w-full flex justify-between items-center'>
                <Link href="/" className="flex tracking-wider items-center md:items-baseline capitalize gap-2">
                    <img ref={logoImg} src={logoSrc} alt="Company Logo" className='w-[36px]' />
            ...
            <div className='py-4 px-4 md:px-6 text-black w-full flex justify-between items-center'>
                <Link href="/" className="flex tracking-wider items-end capitalize gap-2">
                    <img src='logo org.svg' alt="Company Logo" className='w-[36px]' />
                    <p className='md:text-4xl flex flex-col items-baseline text-black md:flex-row gap-1 leading-4'>
                        <span className={`${myFont.className} text-6xl text-orange-500`}>DHUNI</span>
                        CONSTRUCTIONS PVT. LTD.
                    </p>
                </Link>
                ...
                <div className="menu-copy p-8 flex h-full w-full flex-col">
                    <div className="menu-links group flex flex-col gap-10">
                        {navItems.map((link, index) => (
                            <div className="menu-links-item" key={index}>
                                <div className="menu-link-item-holder" onClick={toggleMenu}>
                                    <Link href={link.path} className={`menu-link text-black hover:text-gray transition tracking-tight font-bold md:text-10xl text-[46px] leading-[85%] uppercase`}>
                                        {link.label}
                                    </Link>
                                </div>
                            </div>
